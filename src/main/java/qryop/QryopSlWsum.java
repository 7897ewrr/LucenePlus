      ...
      while (p1 < result.docScores.scores.size()) {
        if (this.daatPtrs.get(i).scoreList.scores.size() < 1) {
          ...
          result2.docScores.add(result.docScores.getDocId(p1),
              result.docScores.getDocIdScore(p1) + score * (weight.get(i) / weightSum));
          p1++;
          ...
          result2.docScores.add(result.docScores.getDocId(p1), result.docScores.getDocIdScore(p1)
              + this.daatPtrs.get(i).scoreList.getDocIdScore(p2) * (weight.get(i) / weightSum));
          p1++;
          p2++;
  ...
  public double getDefaultScore(RetrievalModel r, long docid) throws IOException {
    ...
    if (r instanceof RetrievalModelIndri) {
      ...
      for (int i = 0; i < this.args.size(); i++) {
        score = score + ((QryopSl) this.args.get(i)).getDefaultScore(r, docid) * (weight.get(i)
            / weightSum);
      }
      return score;
    }
